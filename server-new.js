// NEW SERVER - NOWPayments API testing
...
app.post('/api/create-nowpayments-payment', async (req, res) => {
    try {
        ...
        console.log('🚀🚀🚀 NEW SERVER - Creating payment:', { amount, order_id, crypto_type });
        console.log('🚀🚀🚀 THIS IS THE NEW SERVER CODE RUNNING!');
        ...
        // Handle stablecoins with 1:1 rate
        let cryptoAmount;
        if (crypto_type === 'usdc' || crypto_type === 'usdt') {
            cryptoAmount = numericAmount.toFixed(2);
            console.log(`✅ NEW SERVER - Using 1:1 rate for ${crypto_type}: ${cryptoAmount}`);
        } else {
            ...
            try {
                ...
                if (estimateResponse.ok) {
                    ...
                    console.log(`✅ NEW SERVER - Got estimate for ${crypto_type}: ${cryptoAmount}`);
                } else {
                    throw new Error('Estimate failed');
                }
            } catch (error) {
                console.log(`⚠️ NEW SERVER - Estimate failed for ${crypto_type}, using fallback`);
                const fallbackRates = {
        ...
        console.log('🚀 NEW SERVER - Creating payment with request:', paymentRequest);
        ...
        if (!paymentResponse.ok) {
            ...
            console.error('❌ NEW SERVER - Payment API error:', errorText);
            throw new Error(`Payment API error: ${paymentResponse.status} - ${errorText}`);
        }
        
        const paymentResult = await paymentResponse.json();
        console.log('✅ NEW SERVER - Payment created:', paymentResult);
        ...
        console.error('❌ NEW SERVER - Error:', error);
        res.status(500).json({
            success: false,
            error: error.message
        });
...
app.post('/api/nowpayments-webhook', async (req, res) => {
    try {
        ...
        console.log('NEW SERVER - Webhook received:', webhookData);
...
app.listen(PORT, () => {
    ...
    console.log(`   - Added 1:1 rate for USDC/USDT`);
