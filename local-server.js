// Local development server for NOWPayments API testing
...
async function createNOWPaymentsPayment(paymentData) {
    try {
        ...
        // For stablecoins like USDC/USDT, use 1:1 rate and skip estimate API
        if (crypto_type === 'usdc' || crypto_type === 'usdt') {
            ...
            try {
                ...
                if (estimateResponse.ok) {
                    ...
                    console.log(`Direct estimate failed for ${crypto_type}, using fallback calculation...`);
                    throw new Error('Direct estimate not available');
                }
            } catch (error) {
                console.log(`Estimate API failed for ${crypto_type}, using fallback calculation...`);
